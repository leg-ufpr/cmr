...
espaço na núvem mais conhecido que existe. Se você usar o Dropbox, note
que existe um diretório chamado `Public/`. Como sugere, o conteúdo desse
diretório é publico. Chique o botão direito sobre algum arquivo para
pegar o link de acesso (`Dropbox > Copy Public Link`). Envie esse link
para seus destinatários. No nosso caso, podemos hospedar um arquivo
`.txt` (embora sirva para qualquer outro também) e ler de dentro de uma
sessão R. Nesse diretório tem um arquivo chamado `How to use the Public
folder.txt` que informa como usar o recurso. Para demonstar, vou pegar o
link desse arquivo e ler com o R.
...
Como diz o texto, apenas links para arquivos, e não diretórios,
existem. Caso precise enviar muitos arquivos, compacte o diretório (faça
um `.zip`, `.rar`, etc).

*Importante*: No seu CMR já use o link dentro de uma função `read.*()`,
ou outra, enfim, isso depende do tipo de dado. É sempre recomendável que
você indique o *encoding* durante a leitura.

Mais sobre a `Public/` você encontra em
[What is the Public folder for?](https://www.dropbox.com/en/help/16)

É <https://www.copy.com/> o um serviço equivalente.

**Infotomb**

Infotomb (<https://infotomb.com/>) é um repositório de arquivos
anônimo. Arquivos textos podem ser disponibilizados pelo site, bem como
qualquer outro formato (jgp, png, pdf, etc). Os arquivos tem um tempo de
vida que pode ser modificado (1) após o upload. Também é possível
atribuir senha aos arquivos (2) para restringir o acesso. O mais
interessante, no entanto, é que o arquivo e pode ser lido diretamente
pelo link (3). Muitos serviços de hopedagem não dão uma url do arquivo
que permite a leitura por ele, apenas dão um link que abre uma caixa de
download, você clica em OK e baixa. Parece exagero de preocupação mas
com o link para ler direto a coisa vai mais rápido, se resolve na sessão
R. Com botão de download a pessoa tem que salvar o arquivo, mudar o
diretório de trabalho, ler e depois lembrar de deletar o arquivo.
...
No <http://www.datafilehost.com/> você sobe o arquivo e links para
download são gerados. Diferente do anterior, não é dado link para
download direto e sim para uma caixa de diálogo com botão de
download. Os arquivos são removidos quando ultrapassam 90 dias sem
downloads.
